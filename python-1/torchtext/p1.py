...
def train(dataloader):
    ...
    for idx, (label, text, offsets) in enumerate(tqdm.tqdm(dataloader, desc="Training", colour="red")):
        optimizer.zero_grad()
...
def evaluate(dataloader):
    ...
    with torch.no_grad():
        for idx, (label, text, offsets) in enumerate(tqdm.tqdm(dataloader, desc="Evaluating", colour="green")):
            predicted_label = model(text, offsets)
            loss = criterion(predicted_label, label)
            total_acc += (predicted_label.argmax(1) == label).sum().item()
            total_count += label.size(0)
