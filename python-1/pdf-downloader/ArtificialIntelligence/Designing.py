...
if not os.path.exists('Designing'):
    os.mkdir('Designing')

# Get the url of the website
url = 'https://theswissbay.ch/pdf/Gentoomen%20Library/Game%20Development/Designing/'
...
for link in tqdm.tqdm(links):
    ...
    if link_text.endswith(tuple(extensions)):
        ...
        if os.path.exists(f'Designing/{link_text}'):
            continue

        # Temporary file name
        unconfirmed_file = f'Designing/{link_text}.unconfirmed'

        # Actual file name
        confirmed_file = f'Designing/{link_text}'
